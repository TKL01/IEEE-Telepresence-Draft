...
Even though these neural networks excel in a static environment, they are constrained in a dynamic and busy environment where people, animals or other objects such as cars are moving.
...
\subsection{Web Framework, Wiederholung aus MMAR24 Paper?}
The web framework is used to control and monitor the Husky, which runs on ROS2 Humble and is platform-independent. 
Due to the web-based approach, all devices with a browser can access the app.  
\begin{figure}[htbp]
    \centerline{\includegraphics[width=8.9cm]{images/ROS_Web_App_Architecture.pdf}}
    \caption{Backend architecture of the web app with a Screenshot of the frontend}
    \label{fig:userapp}
\end{figure}
